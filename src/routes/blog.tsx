...
function BlogPage() {
  ...
  const loading    = allBlogs.length === 0;
    ...
    <>
      ...
      <Section>
        {loading ? (
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {[1,2,3,4,5,6].map((i) => (
              <div key={i} className="animate-pulse rounded-2xl bg-card shadow-card overflow-hidden">
            ...
            {posts.map((p) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <button
                    onClick={() => alert(`Full article: "${p.title}" — coming soon!`)}
      ...
      <section className="bg-secondary/40">
        <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-6 lg:py-24">
          <span className="text-xs font-semibold uppercase tracking-widest text-primary">Newsletter</span>
          <h2 className="mt-3 font-display text-3xl font-semibold sm:text-4xl">Get health tips in your inbox</h2>
          <p className="mt-4 text-muted-foreground">Join 1,200+ readers. No spam — just one useful article every two weeks.</p>

          {nlStatus === "success" ? (
            <div className="mt-8 rounded-2xl bg-primary-soft px-6 py-4 text-primary">✓ You're subscribed! Watch your inbox.</div>
          ) : (
            <form onSubmit={handleNewsletter} className="mt-8 flex flex-col gap-3 sm:flex-row">
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required className="flex-1 rounded-full border border-input bg-background px-5 py-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
              <button type="submit" disabled={nlStatus === "loading"} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-soft transition-transform hover:scale-[1.02] disabled:opacity-60">
                {nlStatus === "loading" ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-primary-foreground border-t-transparent" /> : <Send className="h-4 w-4" />}
                Subscribe
              </button>
            </form>
          )}
