  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:20 }}>
      <div style={{ background:"#fff", borderRadius:20, padding:"36px 32px", width:560, maxWidth:"100%", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ marginBottom:16 }}><label style={lbl}>Excerpt *</label><textarea value={excerpt} onChange={e=>setExcerpt(e.target.value)} rows={3} placeholder="Short description…" style={{ ...inp, resize:"vertical" }} /></div>
        <div style={{ marginBottom:16 }}>
        ...
        <p style={{ fontSize:12, color:"#6B9E8A", marginBottom:20 }}>💡 <strong>Published</strong> posts appear on /blog immediately.</p>
        <div style={{ display:"flex", gap:12, justifyContent:"flex-end" }}>
          ...
          <button onClick={handleSave} disabled={saving} style={{ padding:"11px 24px", borderRadius:10, background:saving?"#6EC4A7":"#1D9E75", color:"#fff", fontWeight:700, fontSize:14, border:"none", cursor:saving?"not-allowed":"pointer" }}>
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:20 }}>
      <div style={{ background:"#fff", borderRadius:20, padding:"36px 32px", width:520, maxWidth:"100%", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ display:"flex", gap:12 }}>
          {appt.status!=="accepted" && <button onClick={onAccept} style={{ flex:1, padding:12, borderRadius:10, background:"#1D9E75", color:"#fff", fontWeight:700, fontSize:14, border:"none", cursor:"pointer" }}>✓ Accept & Email Patient</button>}
          {appt.status!=="rejected" && <button onClick={onReject} style={{ flex:1, padding:12, borderRadius:10, background:"#fff", color:"#B91C1C", fontWeight:700, fontSize:14, border:"2px solid #FEE2E2", cursor:"pointer" }}>✕ Reject & Email Patient</button>}
...
function AppointmentsTab({ showToast }: { showToast:(m:string)=>void }) {
  ...
  useEffect(() => {
    fetchAppointments().then(() => setLoadingAppts(false));
  }, []);

  // ✅ Updates Supabase status + sends email to patient
  const updateStatus = async (appt: Appointment, status: AppointmentStatus) => {
    ...
    // Send confirmation or rejection email to patient
    await sendStatusEmail(appt, status);

    showToast(
      status==="accepted"
        ? `✓ Accepted! Confirmation email sent to ${appt.email}`
        : `Rejected. Email notification sent to ${appt.email}`
    );
  };
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", borderRadius:16, boxShadow:"0 2px 12px rgba(15,110,86,0.06)", overflow:"hidden" }}>
        {loadingAppts ? (
          <div style={{ padding:"60px 24px", textAlign:"center", color:"#9CA3AF", fontSize:15 }}>Loading appointments…</div>
        ) : filtered.length===0 ? (
                ...
                >
                  ...
                  <td style={{ padding:"14px 18px" }}>
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                      ...
                      {a.status==="pending" && <>
                        <button onClick={()=>updateStatus(a,"accepted")} style={{ padding:"6px 12px", borderRadius:7, border:"none", background:"#1D9E75", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer" }}>✓ Accept</button>
                        <button onClick={()=>updateStatus(a,"rejected")} style={{ padding:"6px 12px", borderRadius:7, border:"none", background:"#FEE2E2", color:"#B91C1C", fontSize:12, fontWeight:700, cursor:"pointer" }}>✕ Reject</button>
                      </>}
      ...
      {deleteId!==null && <ConfirmModal title="Remove this appointment?" body="This will permanently delete the record." onConfirm={()=>removeAppt(deleteId)} onCancel={()=>setDeleteId(null)} />}
    </div>
  );
}
...
function BlogTab({ showToast }: { showToast:(m:string)=>void }) {
  ...
  useEffect(() => {
    fetchBlogs().then(() => setLoadingBlogs(false));
  }, []);
  ...
  const handleAdd    = async (data:Omit<BlogPost,"id"|"color">) => { await addBlog(data); setShowForm(false); showToast(data.status==="published"?"✓ Blog published! Now live on /blog":"✓ Draft saved."); };
  ...
  const handleDelete = async (id:string) => { await deleteBlog(id); setDeleteId(null); showToast("Blog post deleted."); };
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", borderRadius:16, boxShadow:"0 2px 12px rgba(15,110,86,0.06)", overflow:"hidden" }}>
        {loadingBlogs ? (
          <div style={{ padding:"60px 24px", textAlign:"center", color:"#9CA3AF", fontSize:15 }}>Loading blog posts…</div>
        ) : filtered.length===0 ? (
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #E6F4ED", padding:"0 32px", display:"flex", gap:4 }}>
        {([{key:"appointments",label:"📅  Appointments",badge:pendingCount},{key:"blogs",label:"✍️  Blog Posts",badge:0}] as const).map(t=>(
          <button key={t.key} onClick={()=>setActiveTab(t.key)} style={{ padding:"16px 20px", background:"none", border:"none", borderBottom:`3px solid ${activeTab===t.key?"#1D9E75":"transparent"}`, color:activeTab===t.key?"#0F6E56":"#6B7280", fontWeight:activeTab===t.key?700:500, fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", gap:8, transition:"all 0.15s" }}>
            {t.label}
            {t.badge>0 && <span style={{ background:"#EF4444", color:"#fff", fontSize:11, fontWeight:700, padding:"1px 7px", borderRadius:20 }}>{t.badge}</span>}
          </button>
        ))}
      </div>
        ...
        <div style={{ marginBottom:24 }}>
          ...
          <p style={{ fontSize:14, color:"#6B9E8A" }}>
            {activeTab==="appointments"
              ? "Accept or reject bookings — patient gets an email automatically."
              : "Published posts appear immediately on /blog."}
          </p>
        </div>
