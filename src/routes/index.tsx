...
const testimonials = [
  { name: "Anna M.", text: "After three sessions my back pain was gone. Truly life-changing care." },
  { name: "Lukas R.", text: "Professional, warm and incredibly knowledgeable. Highly recommended." },
  { name: "Sofia D.", text: "Helped me recover from knee surgery faster than I expected." },
];
    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {testimonials.map((t) => (
            <div key={t.name} className="rounded-2xl bg-card p-7 shadow-card">
              <div className="flex gap-1 text-primary">
                {Array.from({ length: 5 }).map((_, i) => <Star key={i} className="h-4 w-4 fill-current" />)}
              </div>
              <p className="mt-4 text-foreground/90">"{t.text}"</p>
              <div className="mt-5 text-sm font-semibold text-muted-foreground">— {t.name}</div>
            </div>
          ))}
        </div>
      </section>
...
// unused icon placeholders so tree-shake keeps types
void Hand; void Home;
